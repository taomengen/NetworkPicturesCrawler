/*
如果出现编译错误：const wchar_t *" 类型的实参与 "LPCSTR" 类型的形参不兼容
解决方法：
...
using namespace std;  
#define DEFAULT_PAGE_BUF_SIZE 1000;  
...
{  
	...
	wstring wfoldname;
	StringToWString(foldname,wfoldname);
    if(!CreateDirectory(wfoldname.c_str(),NULL ))  
...
{  
    ...
    // 创建文件夹，保存图片和网页文本文件  
    CreateDirectory(L"./img",0);  
    CreateDirectory(L"./html",0);  
    // 遍历的起始地址  
    string urlStart = "http://www.piaoliang.com/shenghuo/";
    // 使用广度遍历  
